...
# Batch processing logging — shows skipped/processed/failed day counts during batch runs
VERBOSE_BATCH_LOGGING = True
...
TOBAC_THRESHOLDS = (
    ...
    40.0,
    45.0,
)  # detect convective cells with composite reflectivity >= 35 dBZ
TOBAC_SEGMENTATION_THRESHOLD = 35.0  # outer cell boundary uses the same 35 dBZ minimum
TOBAC_MIN_PIXELS = 12  # 12 pixels at 500 m grid spacing = 3 km^2 minimum area
...
CFAD_ENABLED = True
# Target selection mode for CFAD profiles:
#   'index' = Nth observed scan in each track (0 = initiation)
#   'elapsed_minutes' = observation nearest N minutes after initiation
#   'lifetime_fraction' = observation nearest fraction of observed track lifetime (0..1)
CFAD_TARGET_MODE = "index"
CFAD_TARGET_INDICES = [0]
CFAD_ISOLATED_ONLY = True  # use only tracks with no TOBAC merge/split relationships
CFAD_KDP_CALC = True
CFAD_NORM_OPT = 2
...
REGIONAL_CONFIG = {
    ...
    # Fixed regional-axis wind configuration for Shepherd/Burian-style controls.
    # ERA5 JJA 2017-2025 mean at 700 hPa over the Houston box: wind FROM 143.85°
    # (southeasterly), blowing TO/downwind 323.85° (northwesterly).
    "wind_data_path": f"{OUTPUT_DIR}/wind_data/wind_data_1.nc",
    "default_wind_direction": 143.85,  # meteorological FROM direction (degrees)
    "temporal_wind": 323.85,  # fixed downwind bearing used to orient regions
    "pressure_level": 700,  # hPa
    "temporal_avg": "overall",  # fixed JJA climatological mean
    # Distance parameters
