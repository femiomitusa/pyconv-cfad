...
readonly HRRR_DOWNLOAD_SCRIPT="environmental_analysis/stage0_download_hrrr.py"
readonly HRRR_ATTACHMENT_SCRIPT="environmental_analysis/attach_hrrr_environment.py"
...
KEYS = [
    ...
    'RUN_REGIONAL_FILTERING','RUN_HRRR_DOWNLOAD','RUN_HRRR_ATTACHMENT',
    'TARGET_YEAR','TARGET_MONTH','TARGET_DAY',
    'TARGET_MODE','CFAD_OUTPUT_DIR','CFAD_TARGET_MODE'
]
...
PY
	...
	if [ "$RUN_HRRR_DOWNLOAD" = "True" ]; then TOTAL_STAGES=$((TOTAL_STAGES + 1)); fi
	if [ "$RUN_HRRR_ATTACHMENT" = "True" ]; then TOTAL_STAGES=$((TOTAL_STAGES + 1)); fi
...
print_loaded_configuration() {
	...
	echo "  HRRR download enabled: ${RUN_HRRR_DOWNLOAD}"
	echo "  HRRR attachment enabled: ${RUN_HRRR_ATTACHMENT}"
	echo "  CFAD analysis enabled: ${RUN_CFAD_ANALYSIS}"
	echo "  Target mode: ${TARGET_MODE}
  CFAD target mode: ${CFAD_TARGET_MODE:-index}"
}
...
validate_radar_packages() {
	...
	local missing
	missing=$(check_python_modules "pyart,zarr,scipy")
...
validate_hrrr_packages() {
	if [ "$RUN_HRRR_DOWNLOAD" != "True" ] && [ "$RUN_HRRR_ATTACHMENT" != "True" ]; then return; fi

	local missing
	missing=$(check_python_modules "numpy,pandas,xarray,scipy")
	if [ -n "$missing" ]; then
		fatal_error "HRRR/environment packages not found: $missing"
	fi

	if [ "$RUN_HRRR_DOWNLOAD" = "True" ]; then
		missing=$(check_python_modules "cfgrib,eccodes,herbie")
		if [ -n "$missing" ]; then
			fatal_error "HRRR download packages not found: $missing. Install with: python -m pip install cfgrib eccodes herbie-data"
		fi
	fi
	print_success "HRRR/environment packages are installed."
}
...
validate_cfad_packages() {
	...
	local missing
	missing=$(check_python_modules "numpy,xarray,matplotlib,zarr")
...
validate_required_scripts() {
	...
	if [ "$RUN_HRRR_DOWNLOAD" = "True" ] && [ ! -f "$HRRR_DOWNLOAD_SCRIPT" ]; then
		fatal_error "HRRR download script not found at $HRRR_DOWNLOAD_SCRIPT"
	fi

	if [ "$RUN_HRRR_ATTACHMENT" = "True" ] && [ ! -f "$HRRR_ATTACHMENT_SCRIPT" ]; then
		fatal_error "HRRR attachment script not found at $HRRR_ATTACHMENT_SCRIPT"
	fi
...
regional_zarr_dates() {
	find "$BASE_DATA_DIR/Arrays_Regional/all" -name "KHGX*_regional.zarr" -type d 2>/dev/null |
		sed -E 's/.*KHGX([0-9]{8})_regional\.zarr/\1/' |
		sort -u |
		awk '{print substr($0,1,4) "-" substr($0,5,2) "-" substr($0,7,2)}'
}

run_hrrr_download() {
	if [ "$RUN_HRRR_DOWNLOAD" != "True" ]; then
		print_section "HRRR ENVIRONMENT DOWNLOAD"
		print_warning "HRRR download is disabled in config.py."
		return
	fi

	stage_start "HRRR ENVIRONMENT DOWNLOAD"
	if [ "$TARGET_MODE" == "True" ]; then
		echo "Downloading HRRR environment for target date: $(format_target_date)"
		python3 "$HRRR_DOWNLOAD_SCRIPT" --date "$(format_target_date)"
	else
		echo "Downloading HRRR environment for regional Zarr days..."
		regional_zarr_dates | while read -r date; do
			[ -n "$date" ] || continue
			python3 "$HRRR_DOWNLOAD_SCRIPT" --date "$date"
		done
	fi
	stage_end "HRRR ENVIRONMENT DOWNLOAD" "true"
}

run_hrrr_attachment() {
	if [ "$RUN_HRRR_ATTACHMENT" != "True" ]; then
		print_section "HRRR ENVIRONMENT ATTACHMENT"
		print_warning "HRRR attachment is disabled in config.py."
		return
	fi

	stage_start "HRRR ENVIRONMENT ATTACHMENT"
	if [ "$TARGET_MODE" == "True" ]; then
		echo "Sampling HRRR fields with cell segmentation footprints for target date: $(format_target_date)"
		python3 "$HRRR_ATTACHMENT_SCRIPT" --date "$(format_target_date)"
	else
		echo "Sampling HRRR fields with cell segmentation footprints for regional Zarr days..."
		regional_zarr_dates | while read -r date; do
			[ -n "$date" ] || continue
			python3 "$HRRR_ATTACHMENT_SCRIPT" --date "$date"
		done
	fi
	stage_end "HRRR ENVIRONMENT ATTACHMENT" "true"
}
...
print_output_locations() {
	...
	if [ "$RUN_REGIONAL_FILTERING" = "True" ]; then
		echo "  Regional data: $BASE_DATA_DIR/Arrays_Regional/[region]/"
	fi
	if [ "$RUN_HRRR_DOWNLOAD" = "True" ]; then
		echo "  HRRR environment: $BASE_DATA_DIR/hrrr/"
	fi
	if [ "$RUN_CFAD_ANALYSIS" = "True" ]; then
